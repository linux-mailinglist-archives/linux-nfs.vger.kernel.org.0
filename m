...
Received: from out1.vger.email (out1.vger.email [IPv6:2620:137:e000::1:20])
	by mail.lfdr.de (Postfix) with ESMTP id A35D35F6B76
	for <lists+linux-nfs@lfdr.de>; Thu,  6 Oct 2022 18:20:55 +0200 (CEST)
Received: (majordomo@vger.kernel.org) by vger.kernel.org via listexpand
        id S230408AbiJFQUy (ORCPT <rfc822;lists+linux-nfs@lfdr.de>);
        Thu, 6 Oct 2022 12:20:54 -0400
Received: from lindbergh.monkeyblade.net ([23.128.96.19]:49720 "EHLO
        lindbergh.monkeyblade.net" rhost-flags-OK-OK-OK-OK) by vger.kernel.org
        with ESMTP id S230478AbiJFQUw (ORCPT
        <rfc822;linux-nfs@vger.kernel.org>); Thu, 6 Oct 2022 12:20:52 -0400
Received: from dfw.source.kernel.org (dfw.source.kernel.org [139.178.84.217])
        by lindbergh.monkeyblade.net (Postfix) with ESMTPS id E98C2ACA12
        for <linux-nfs@vger.kernel.org>; Thu,  6 Oct 2022 09:20:50 -0700 (PDT)
Received: from smtp.kernel.org (relay.kernel.org [52.25.139.140])
        (using TLSv1.2 with cipher ECDHE-RSA-AES256-GCM-SHA384 (256/256 bits))
        (No client certificate requested)
        by dfw.source.kernel.org (Postfix) with ESMTPS id 6B26261A10
        for <linux-nfs@vger.kernel.org>; Thu,  6 Oct 2022 16:20:50 +0000 (UTC)
Received: by smtp.kernel.org (Postfix) with ESMTPSA id BE416C433C1
        for <linux-nfs@vger.kernel.org>; Thu,  6 Oct 2022 16:20:49 +0000 (UTC)
Subject: [PATCH v2 7/9] NFSD: Use rhashtable for managing nfs4_file objects
From:   Chuck Lever <chuck.lever@oracle.com>
To:     linux-nfs@vger.kernel.org
Date:   Thu, 06 Oct 2022 12:20:48 -0400
Message-ID: <166507324882.1802.884870684212914640.stgit@manet.1015granger.net>
...
index 2b850de288cf..bb52510a6a83 100644
...
+static void remove_nfs4_file(struct nfs4_file *fp);
...
-	hlist_add_head_rcu(&fp->fi_hash, &file_hashtbl[hashval]);
 ...
 void
@@ -4626,71 +4688,92 @@ move_to_close_lru(struct nfs4_ol_stateid *s, struct net *net)
...
-	struct nfs4_file *fp;
+/*
+ * On hash insertion, identify any inode aliases. They will
+ * all be in the same hash bucket because we hash by the
+ * address of the struct inode.
+ */
+static void check_nfs4_file_aliases(struct nfs4_file *new,
...
-				return fp;
+	/*
+	 * rhashtable avoids large buckets, thus this loop stays
+	 * efficient.
+	 */
+	rcu_read_lock();
...
+find_or_insert_nfs4_file(struct nfs4_file *new, const struct svc_fh *fhp)
 {
-	struct nfs4_file *fp;
-	unsigned int hashval = file_hashval(fh);
+	struct nfs4_file *fi;
...
+		err = rhashtable_lookup_insert_key(&nfs4_file_rhashtbl, fhp,
+						   &new->fi_rhash,
+						   nfs4_file_rhash_params);
+		if (!err) {
+			fi = new;
+			check_nfs4_file_aliases(fi, fhp);
+			break;
+		}
+		if (err != -EEXIST) {
+			fi = NULL;
...
+			       nfs4_file_rhash_params);
 ...
 /*
@@ -4703,9 +4786,10 @@ nfs4_share_conflict(struct svc_fh *current_fh, unsigned int deny_type)
 	struct nfs4_file *fp;
 	__be32 ret = nfs_ok;
...
@@ -5548,7 +5632,9 @@ nfsd4_process_open2(struct svc_rqst *rqstp, struct svc_fh *current_fh, struct nf
 	 * and check for delegations in the process of being recalled.
 	 * If not found, create the nfs4_file struct
 	 */
-	fp = find_or_add_file(open->op_file, current_fh);
+	fp = find_or_insert_nfs4_file(open->op_file, current_fh);
+	if (unlikely(!fp))
+		return nfserr_jukebox;
 	if (fp != open->op_file) {
 		status = nfs4_check_deleg(cl, open, &dp);
 		if (status)
@@ -7905,10 +7991,16 @@ nfs4_state_start(void)
 {
 	int ret;
...
@@ -7939,6 +8031,7 @@ nfs4_state_shutdown_net(struct net *net)
