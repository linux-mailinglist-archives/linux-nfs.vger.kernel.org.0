...
Received: from out1.vger.email (out1.vger.email [IPv6:2620:137:e000::1:20])
	by mail.lfdr.de (Postfix) with ESMTP id 347126C3754
	for <lists+linux-nfs@lfdr.de>; Tue, 21 Mar 2023 17:47:04 +0100 (CET)
Received: (majordomo@vger.kernel.org) by vger.kernel.org via listexpand
        id S230024AbjCUQrC (ORCPT <rfc822;lists+linux-nfs@lfdr.de>);
        Tue, 21 Mar 2023 12:47:02 -0400
Received: from lindbergh.monkeyblade.net ([23.128.96.19]:33510 "EHLO
        lindbergh.monkeyblade.net" rhost-flags-OK-OK-OK-OK) by vger.kernel.org
        with ESMTP id S230134AbjCUQrB (ORCPT
        <rfc822;linux-nfs@vger.kernel.org>); Tue, 21 Mar 2023 12:47:01 -0400
Received: from dfw.source.kernel.org (dfw.source.kernel.org [139.178.84.217])
        by lindbergh.monkeyblade.net (Postfix) with ESMTPS id 47F4F1284C
        for <linux-nfs@vger.kernel.org>; Tue, 21 Mar 2023 09:46:49 -0700 (PDT)
Received: from smtp.kernel.org (relay.kernel.org [52.25.139.140])
        (using TLSv1.2 with cipher ECDHE-RSA-AES256-GCM-SHA384 (256/256 bits))
        (No client certificate requested)
        by dfw.source.kernel.org (Postfix) with ESMTPS id C7A5E61D38
        for <linux-nfs@vger.kernel.org>; Tue, 21 Mar 2023 16:46:48 +0000 (UTC)
Received: by smtp.kernel.org (Postfix) with ESMTPSA id AF0F9C433D2;
        Tue, 21 Mar 2023 16:46:47 +0000 (UTC)
DKIM-Signature: v=1; a=rsa-sha256; c=relaxed/simple; d=kernel.org;
        s=k20201202; t=1679417208;
        bh=WDfHlVO2st+iOevzwE/nDW5RV1C6RKMK9eSTNsWcM3w=;
        h=Subject:From:To:Cc:Date:In-Reply-To:References:From;
        b=
Message-ID: <27bb222567051576261c950f039247ad5fa064ea.camel@kernel.org>
...
Cc:     Chuck Lever <cel@kernel.org>,
        ...
        <kernel-tls-handshake@lists.linux.dev>
Date:   Tue, 21 Mar 2023 12:46:46 -0400
In-Reply-To: <CB9BF445-99C3-4EEA-9B7A-9DF4CE5659F3@oracle.com>
References: <167932094748.3131.11264549266195745851.stgit@manet.1015granger.net>
         ...
         <CB9BF445-99C3-4EEA-9B7A-9DF4CE5659F3@oracle.com>
...
> > > > >=20
> > > > > This patch adds opportunitistic RPC-with-TLS to the Linux in-kern=
el
...
> > > > > #define XPT_KILL_TEMP   13		/* call xpo_kill_temp_xprt before clo=
sing */
> > > > > #define XPT_CONG_CTRL	14		/* has congestion control */
> > > > > +#define XPT_HANDSHAKE	15		/* xprt requests a handshake */
> > > > > +#define XPT_TLS_SESSION	16		/* transport-layer security establis=
hed */
> > > > > +#define XPT_PEER_AUTH	17		/* peer has been authenticated */
> > > > >=20
> > > > > 	struct svc_serv		*xpt_server;	/* service for transport */
> > > > > 	atomic_t    	    	xpt_reserved;	/* space on outq that is rsvd */
> > > > > diff --git a/include/linux/sunrpc/svcsock.h b/include/linux/sunrp=
c/svcsock.h
...
> > > > > @@ -427,7 +427,7 @@ static bool svc_xprt_ready(struct svc_xprt *x=
prt)
> > > > >=20
> > > > > 	if (xpt_flags & BIT(XPT_BUSY))
> > > > > 		return false;
> > > > > -	if (xpt_flags & (BIT(XPT_CONN) | BIT(XPT_CLOSE)))
> > > > > +	if (xpt_flags & (BIT(XPT_CONN) | BIT(XPT_CLOSE) | BIT(XPT_HANDS=
HAKE)))
> > > > > 		return true;
> > > > > 	if (xpt_flags & (BIT(XPT_DATA) | BIT(XPT_DEFERRED))) {
> > > > > 		if (xprt->xpt_ops->xpo_has_wspace(xprt) &&
> > > > > @@ -829,6 +829,9 @@ static int svc_handle_xprt(struct svc_rqst *r=
qstp, struct svc_xprt *xprt)
...
> > > > > diff --git a/net/sunrpc/svcauth_unix.c b/net/sunrpc/svcauth_unix.=
c
...
> > > > >=20
> > > > > static struct svc_sock *svc_setup_socket(struct svc_serv *, struc=
t socket *,
> > > > > 					 int flags);
...
> > > > > 	}
> > > > > @@ -397,6 +402,89 @@ static void svc_tcp_kill_temp_xprt(struct sv=
c_xprt *xprt)
...
> > > > > + * @peerid: serial number of key containing the remote peer's id=
entity
> > > > > + *
> > > > > + * If a security policy is specified as an export option, we don=
't
> > > > > + * have a specific export here to check. So we set a "TLS sessio=
n
> > > > > + * is present" flag on the xprt and let an upper layer enforce l=
ocal
> > > > > + * security policy.
> > > > > + */
> > > > > +static void svc_tcp_handshake_done(void *data, int status, key_s=
erial_t peerid)
> > > > > +{
> > > > > +	struct svc_xprt *xprt =3D data;
> > > > > +	struct svc_sock *svsk =3D container_of(xprt, struct svc_sock, s=
k_xprt);
> > > > > +
> > > > > +	if (!status) {
...
> > > > > +
> > > > > +/**
> > > > > + * svc_tcp_handshake - Perform a transport-layer security handsh=
ake
...
> > > > > +	}
> > > > > +
> > > > > +	ret =3D wait_for_completion_interruptible_timeout(&svsk->sk_han=
dshake_done,
> > > > > +							SVC_HANDSHAKE_TO);
> > > >=20
...
> > > > with DoS attacks here? Could a client initiate handshakes and then =
stop
...
> > > > > */
> > > > > @@ -1260,6 +1348,7 @@ static const struct svc_xprt_ops svc_tcp_op=
s =3D {
...
> > > > > @@ -1584,6 +1673,8 @@ static void svc_sock_free(struct svc_xprt *=
xprt)
> > > > > {
> > > > > 	struct svc_sock *svsk =3D container_of(xprt, struct svc_sock, sk=
_xprt);
> > > > >=20
