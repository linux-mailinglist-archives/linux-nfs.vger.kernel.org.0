...
Received: from out1.vger.email (out1.vger.email [IPv6:2620:137:e000::1:20])
	by mail.lfdr.de (Postfix) with ESMTP id 59DFC632AAF
	for <lists+linux-nfs@lfdr.de>; Mon, 21 Nov 2022 18:18:25 +0100 (CET)
Received: (majordomo@vger.kernel.org) by vger.kernel.org via listexpand
        id S231561AbiKURST (ORCPT <rfc822;lists+linux-nfs@lfdr.de>);
        Mon, 21 Nov 2022 12:18:19 -0500
Received: from lindbergh.monkeyblade.net ([23.128.96.19]:35384 "EHLO
        lindbergh.monkeyblade.net" rhost-flags-OK-OK-OK-OK) by vger.kernel.org
        with ESMTP id S231130AbiKURR5 (ORCPT
        <rfc822;linux-nfs@vger.kernel.org>); Mon, 21 Nov 2022 12:17:57 -0500
Received: from dfw.source.kernel.org (dfw.source.kernel.org [139.178.84.217])
        by lindbergh.monkeyblade.net (Postfix) with ESMTPS id 088E0C8C90;
        Mon, 21 Nov 2022 09:16:23 -0800 (PST)
Received: from smtp.kernel.org (relay.kernel.org [52.25.139.140])
        (using TLSv1.2 with cipher ECDHE-RSA-AES256-GCM-SHA384 (256/256 bits))
        (No client certificate requested)
        by dfw.source.kernel.org (Postfix) with ESMTPS id 7C13261367;
...
> > +++ b/fs/cifs/cifssmb.c
> > @@ -15,6 +15,7 @@
> >   /* want to reuse a stale file handle and only the caller knows the fi=
le info */
> > =20
> >  #include <linux/fs.h>
> > +#include <linux/filelock.h>
...
resent
> > + * POSIX byte range locks, BSD (flock) locks, and leases. It's importa=
nt to
> > + * note that the same struct is used to represent both a request for a=
 lock and
> > + * the lock itself, but the same object is never used for both.
> > + *
> > + * FIXME: should we create a separate "struct lock_request" to help di=
stinguish
> > + * these two uses?
> > + *
...
> > +
> > +	const struct file_lock_operations *fl_ops;	/* Callbacks for filesyste=
ms */
> > +	const struct lock_manager_operations *fl_lmops;	/* Callbacks for lock=
managers */
> > +	union {
...
> > +extern int posix_lock_file(struct file *, struct file_lock *, struct f=
ile_lock *);
 ...
 *fl);
> > +extern int __break_lease(struct inode *inode, unsigned int flags, unsi=
gned int type);
...
> > +}
> > +#endif
> > +static inline int fcntl_setlease(unsigned int fd, struct file *filp, l=
ong arg)
> > +{
> > +	return -EINVAL;
> > +}
...
> > +}
> > +
> > +static inline void locks_copy_conflock(struct file_lock *new, struct f=
ile_lock *fl)
> > +{
> > +	return;
> > +}
> > +
> > +static inline void locks_copy_lock(struct file_lock *new, struct file_=
lock *fl)
> > +{
> > +	return;
> > +}
> > +
> > +static inline void locks_remove_posix(struct file *filp, fl_owner_t ow=
ner)
> > +{
> > +	return;
> > +}
...
> > +}
> > +
> > +static inline int posix_lock_file(struct file *filp, struct file_lock =
*fl,
...
> > +}
> > +
> > +static inline int vfs_test_lock(struct file *filp, struct file_lock *f=
l)
> > +{
> > +	return 0;
> > +}
...
> > +}
> > +
> > +static inline int vfs_cancel_lock(struct file *filp, struct file_lock =
*fl)
> > +{
> > +	return 0;
> > +}
> > +
> > +static inline int locks_lock_inode_wait(struct inode *inode, struct fi=
le_lock *fl)
> > +{
> > +	return -ENOLCK;
> > +}
...
> > +
> > +static inline int locks_lock_file_wait(struct file *filp, struct file_=
lock *fl)
> > +{
...
> > +}
> > +
> > +static inline int try_break_deleg(struct inode *inode, struct inode **=
delegated_inode)
> > +{
> > +	int ret;
> > +
...
> > @@ -1003,132 +1003,11 @@ static inline struct file *get_file(struct fil=
e *f)
...
> >  {
> >  	return f->f_inode;
> > @@ -1353,11 +1027,6 @@ static inline struct dentry *file_dentry(const s=
truct file *file)
