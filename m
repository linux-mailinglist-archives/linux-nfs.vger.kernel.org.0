...
Received: from vger.kernel.org (vger.kernel.org [209.132.180.67])
	by mail.lfdr.de (Postfix) with ESMTP id 20519103E88
	for <lists+linux-nfs@lfdr.de>; Wed, 20 Nov 2019 16:29:27 +0100 (CET)
Received: (majordomo@vger.kernel.org) by vger.kernel.org via listexpand
        id S1729184AbfKTP3W (ORCPT <rfc822;lists+linux-nfs@lfdr.de>);
        Wed, 20 Nov 2019 10:29:22 -0500
Received: from us-smtp-delivery-1.mimecast.com ([205.139.110.120]:41063 "EHLO
        us-smtp-1.mimecast.com" rhost-flags-OK-OK-OK-FAIL) by vger.kernel.org
        with ESMTP id S1730214AbfKTP2A (ORCPT
        <rfc822;linux-nfs@vger.kernel.org>); Wed, 20 Nov 2019 10:28:00 -0500
DKIM-Signature: v=1; a=rsa-sha256; c=relaxed/relaxed; d=redhat.com;
        ...
        bh=YVs5D0YRVQUWfjR5NEe/zy0nv7F5FkjZekaziwTqO7Q=;
        b=SCboSK7vZ5IIQctjGRl49QMB+qO/MxKxmLWB58taGv9P8tirkTQzfHFpsa2A4/TeZ6h769
        YaWUIxLWnZBJNfr4fNdpWlNJ3x+lgaGB2XLyLttaQL3xmSJkZ72NFCq9X3EQ32yegUvYfT
        4YovchMdKtY/Hk/vTnPgaKD1oP5hpqw=
Received: from mimecast-mx01.redhat.com (mimecast-mx01.redhat.com
 [209.132.183.4]) (Using TLS) by relay.mimecast.com with ESMTP id
 us-mta-324-n7CptDjmP6ab8qseGsFx0A-1; Wed, 20 Nov 2019 10:27:54 -0500
Received: from smtp.corp.redhat.com (int-mx05.intmail.prod.int.phx2.redhat.com [10.5.11.15])
        (using TLSv1.2 with cipher AECDH-AES256-SHA (256/256 bits))
        (No client certificate requested)
        by mimecast-mx01.redhat.com (Postfix) with ESMTPS id 567FE80269B;
        Wed, 20 Nov 2019 15:27:53 +0000 (UTC)
Received: from coeurl.usersys.redhat.com (ovpn-123-90.rdu2.redhat.com [10.10.123.90])
        by smtp.corp.redhat.com (Postfix) with ESMTP id C5FE863644;
        Wed, 20 Nov 2019 15:27:52 +0000 (UTC)
Received: by coeurl.usersys.redhat.com (Postfix, from userid 1000)
        id A6A1220AC8; Wed, 20 Nov 2019 10:27:50 -0500 (EST)
From:   Scott Mayhew <smayhew@redhat.com>
To:     anna.schumaker@netapp.com, trond.myklebust@hammerspace.com
Cc:     dhowells@redhat.com, viro@zeniv.linux.org.uk,
        linux-nfs@vger.kernel.org, linux-fsdevel@vger.kernel.org,
        linux-kernel@vger.kernel.org
Subject: [PATCH v5 26/27] NFS: Additional refactoring for fs_context conversion
Date:   Wed, 20 Nov 2019 10:27:49 -0500
Message-Id: <20191120152750.6880-27-smayhew@redhat.com>
In-Reply-To: <20191120152750.6880-1-smayhew@redhat.com>
References: <20191120152750.6880-1-smayhew@redhat.com>
X-Scanned-By: MIMEDefang 2.79 on 10.5.11.15
X-MC-Unique: n7CptDjmP6ab8qseGsFx0A-1
...
---
 ...
 13 files changed, 204 insertions(+), 274 deletions(-)
...
-=09struct nfs_subversion *nfs_mod =3D mount_info->nfs_mod;
 ...
 =09int error;
=20
@@ -959,18 +959,18 @@ struct nfs_server *nfs_create_server(struct nfs_mount=
_info *mount_info)
 =09=09goto error;
=20
...
@@ -978,8 +978,8 @@ struct nfs_server *nfs_create_server(struct nfs_mount_i=
nfo *mount_info)
 =09}
=20
...
index ccbff3a9ccf6..007d058baad1 100644
--- a/fs/nfs/internal.h
+++ b/fs/nfs/internal.h
@@ -11,7 +11,7 @@
...
index 927b0f732148..3e566a632215 100644
...
=20
 ...
 =09if (ret < 0) {
@@ -259,22 +259,22 @@ int nfs_do_submount(struct fs_context *fc)
 =09int ret;
=20
...
=20
 ...
 =09if (IS_ERR(p)) {
@@ -299,7 +299,7 @@ int nfs_submount(struct fs_context *fc, struct nfs_serv=
er *server)
=20
...
@@ -1052,9 +1052,9 @@ static int nfs4_server_common_setup(struct nfs_server=
 *server,
 /*
...
+=09struct nfs_fs_context *ctx =3D nfs_fc2context(fc);
 ...
 =09int error;
=20
@@ -1076,15 +1076,15 @@ static int nfs4_init_server(struct nfs_server *serv=
er,
=20
...
+=09=09=09=09fc->net_ns);
 ...
 =09=09return error;
=20
@@ -1107,10 +1107,9 @@ static int nfs4_init_server(struct nfs_server *serve=
r,
  * Create a version 4 volume record
  * - keyed on server and FSID
  */
...
+=09struct nfs_fs_context *ctx =3D nfs_fc2context(fc);
 ...
 =09bool auth_probe;
 =09int error;
@@ -1121,14 +1120,14 @@ struct nfs_server *nfs4_create_server(struct nfs_mo=
unt_info *mount_info)
=20
...
+=09error =3D nfs4_server_common_setup(server, ctx->mntfh, auth_probe);
 ...
 =09=09goto error;
=20
@@ -1142,9 +1141,9 @@ struct nfs_server *nfs4_create_server(struct nfs_moun=
t_info *mount_info)
 /*
...
+=09struct nfs_fs_context *ctx =3D nfs_fc2context(fc);
 ...
 =09bool auth_probe;
@@ -1154,7 +1153,7 @@ struct nfs_server *nfs4_create_referral_server(struct=
...
@@ -1164,10 +1163,11 @@ struct nfs_server *nfs4_create_referral_server(stru=
...
@@ -1178,10 +1178,11 @@ struct nfs_server *nfs4_create_referral_server(stru=
ct nfs_clone_mount *data,
...
+=09=09=09=09ctx->nfs_server.addrlen,
 ...
 =09=09=09=09parent_server->client->cl_timeout,
@@ -1194,13 +1195,14 @@ struct nfs_server *nfs4_create_referral_server(stru=
ct nfs_clone_mount *data,
 #if IS_ENABLED(CONFIG_SUNRPC_XPRT_RDMA)
 init_server:
 #endif
...
index 04985aebe184..db94e9422501 100644
...
@@ -143,7 +144,6 @@ static int nfs4_validate_fspath(struct dentry *dentry,
 =09}
=20
...
-
 ...
 =09kfree(buf);
 =09kfree(fs_path);
@@ -274,7 +274,6 @@ nfs4_negotiate_security(struct rpc_clnt *clnt, struct i=
node *inode,
...
@@ -310,29 +309,24 @@ static int try_location(struct fs_context *fc,
=20
 =09kfree(fc->source);
...
@@ -445,7 +439,7 @@ int nfs4_submount(struct fs_context *fc, struct nfs_ser=
ver *server)
 =09int ret;
=20
...
index 96edee546b8a..3b27c5e3d781 100644
...
index 3de843888bc0..ed9f215d03ea 100644
