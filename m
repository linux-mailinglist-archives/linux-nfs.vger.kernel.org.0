Return-Path: <linux-nfs+bounces-9503-lists+linux-nfs=lfdr.de@vger.kernel.org>
X-Original-To: lists+linux-nfs@lfdr.de
Delivered-To: lists+linux-nfs@lfdr.de
Received: from am.mirrors.kernel.org (am.mirrors.kernel.org [147.75.80.249])
	by mail.lfdr.de (Postfix) with ESMTPS id 5B763A19A6E
	for <lists+linux-nfs@lfdr.de>; Wed, 22 Jan 2025 22:33:16 +0100 (CET)
Received: from smtp.subspace.kernel.org (relay.kernel.org [52.25.139.140])
	(using TLSv1.2 with cipher ECDHE-ECDSA-AES256-GCM-SHA384 (256/256 bits))
	(No client certificate requested)
	by am.mirrors.kernel.org (Postfix) with ESMTPS id C38D9188D71E
	for <lists+linux-nfs@lfdr.de>; Wed, 22 Jan 2025 21:33:19 +0000 (UTC)
Received: from localhost.localdomain (localhost.localdomain [127.0.0.1])
	by smtp.subspace.kernel.org (Postfix) with ESMTP id 6B9C71C5F20;
	Wed, 22 Jan 2025 21:33:07 +0000 (UTC)
Authentication-Results: smtp.subspace.kernel.org;
	dkim=pass (2048-bit key) header.d=kernel.org header.i=@kernel.org header.b="in+RlMam"
X-Original-To: linux-nfs@vger.kernel.org
Received: from smtp.kernel.org (aws-us-west-2-korg-mail-1.web.codeaurora.org [10.30.226.201])
	(using TLSv1.2 with cipher ECDHE-RSA-AES256-GCM-SHA384 (256/256 bits))
	(No client certificate requested)
	by smtp.subspace.kernel.org (Postfix) with ESMTPS id 4658F1C5F1B
	for <linux-nfs@vger.kernel.org>; Wed, 22 Jan 2025 21:33:06 +0000 (UTC)
Authentication-Results: smtp.subspace.kernel.org; arc=none smtp.client-ip=10.30.226.201
ARC-Seal:i=1; a=rsa-sha256; d=subspace.kernel.org; s=arc-20240116;
	t=1737581587; cv=none; b=rGdv5YHl+RBBL0l57qUp2nS+Kgsa/LgUmIpBwwtPoscM7RNiw916FUrzO8RTHNB1KT8Kzu3S5i8sanhilFbHWVPP4THkIJzomdl+k5mxZ95rdTCDOqJuN33+ApaTRZ9UUIW0IOK3tK7YA2X8oDrEXBLopxF9hmMvfSXDsRZAV9Y=
ARC-Message-Signature:i=1; a=rsa-sha256; d=subspace.kernel.org;
	s=arc-20240116; t=1737581587; c=relaxed/simple;
	bh=RmajvA4BkP4kb20xgu6IAiEdP5BPmpR9a8mDxKJYyKE=;
	h=Message-ID:Subject:From:To:Cc:Date:In-Reply-To:References:
	 Content-Type:MIME-Version; b=inT8Hk9jtqkMQZw3N+4W9suJjo1FZ8zgN5l3iuwPN/3pUrTYOu/9yWQAxypQzbWSfZHc6UEdy4NCdQbP8GsP95rvZIowDAQin96Lz1pq/wFruLsfWSfbx9Ewk81cFiDu3RX8mcu0PZBTVFrX6aVoANvGuNa88sdAczPMZgXGLG8=
ARC-Authentication-Results:i=1; smtp.subspace.kernel.org; dkim=pass (2048-bit key) header.d=kernel.org header.i=@kernel.org header.b=in+RlMam; arc=none smtp.client-ip=10.30.226.201
Received: by smtp.kernel.org (Postfix) with ESMTPSA id 26859C4CED3;
	Wed, 22 Jan 2025 21:33:06 +0000 (UTC)
DKIM-Signature: v=1; a=rsa-sha256; c=relaxed/simple; d=kernel.org;
	s=k20201202; t=1737581586;
	bh=RmajvA4BkP4kb20xgu6IAiEdP5BPmpR9a8mDxKJYyKE=;
	h=Subject:From:To:Cc:Date:In-Reply-To:References:From;
	b=
Message-ID: <b960f2754bac340d76beb56590873171ee097150.camel@kernel.org>
Subject: Re: [PATCH 3/4] nfsd: filecache: change garbage collection list
 management.
From: Jeff Layton <jlayton@kernel.org>
To: NeilBrown <neilb@suse.de>
Cc: Chuck Lever <chuck.lever@oracle.com>, linux-nfs@vger.kernel.org, Olga
 Kornievskaia <okorniev@redhat.com>, Dai Ngo <Dai.Ngo@oracle.com>, Tom
 Talpey <tom@talpey.com>
Date: Wed, 22 Jan 2025 16:33:05 -0500
In-Reply-To: <173758048419.22054.7069906473728685831@noble.neil.brown.name>
References: <>, <2adc801552b18e1f9b006513c8cdcd7881438585.camel@kernel.org>
	 <173758048419.22054.7069906473728685831@noble.neil.brown.name>
Autocrypt: addr=jlayton@kernel.org; prefer-encrypt=mutual;
 keydata=
Content-Type: text/plain; charset="UTF-8"
Content-Transfer-Encoding: quoted-printable
User-Agent: Evolution 3.54.3 (3.54.3-1.fc41) 
Precedence: bulk
X-Mailing-List: linux-nfs@vger.kernel.org
List-Id: <linux-nfs.vger.kernel.org>
List-Subscribe: <mailto:linux-nfs+subscribe@vger.kernel.org>
List-Unsubscribe: <mailto:linux-nfs+unsubscribe@vger.kernel.org>
MIME-Version: 1.0

On Thu, 2025-01-23 at 08:14 +1100, NeilBrown wrote:
> On Thu, 23 Jan 2025, Jeff Layton wrote:
> > On Wed, 2025-01-22 at 14:54 +1100, NeilBrown wrote:
> > > @@ -487,88 +512,32 @@ void nfsd_file_net_dispose(struct nfsd_net *nn)
> > >  		int i;
> > > =20
> > >  		spin_lock(&l->lock);
> > > -		for (i =3D 0; i < 8 && !list_empty(&l->freeme); i++)
> > > -			list_move(l->freeme.next, &dispose);
> >=20
> > While you're in here, could you document why we only take 8 at a time?
> > Maybe even consider turning it into a named constant?
>=20
> I've added a patch to do that.
>=20
> > > @@ -577,9 +546,20 @@ nfsd_file_gc_worker(struct work_struct *work)
> > >  {
> > >  	struct nfsd_fcache_disposal *l =3D container_of(
> > >  		work, struct nfsd_fcache_disposal, filecache_laundrette.work);
> > > -	nfsd_file_gc(l);
> > > -	if (list_lru_count(&l->file_lru))
> > > +
> > > +	spin_lock(&l->lock);
> > > +	list_splice_init(&l->older, &l->freeme);
> > > +	list_splice_init(&l->recent, &l->older);
> > > +	/* We don't know how many were moved to 'freeme' and don't want
> > > +	 * to waste time counting - guess a half.
> > > +	 */
> > > +	l->num_gc /=3D 2;
> >=20
> > Given that you have to manipulate the lists under a spinlock, it
> > wouldn't be difficult or expensive to keep accurate counts. nfsd
> > workloads can be "spiky", so it seems like this method may be wildly
> > inaccurate at times.
>=20
> The only way I can think of to get an accurate count is to iterate the
> list under the spin lock, and the cost of iterating long lists under a
> spinlock is what started this whole exercise.
>=20
> I could keep an accurate count of recent+older+freeme but giving that to
> the shrinker could cause it to shrink too much as the objects on
> "freeme" cannot be freed any faster.
>=20
> Maybe when freeme is not empty I could give zero to the shrinker...
>=20
> Any ideas?


I was thinking we could just increment a counter when adding it to the
list and transplant those counts as needed, but removals are a problem
since you don't necessarily know what list each entry resides on at any
given time, and you can't decrement the right one. It's probably
doable, but you're right that it's complex.

I'm OK with your estimate for now. If it turns out to be a problem
later, we can deal with it then.
--=20
Jeff Layton <jlayton@kernel.org>

