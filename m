...
Received: from out1.vger.email (out1.vger.email [IPv6:2620:137:e000::1:20])
	by mail.lfdr.de (Postfix) with ESMTP id 7A19F74DAE4
	for <lists+linux-nfs@lfdr.de>; Mon, 10 Jul 2023 18:19:01 +0200 (CEST)
Received: (majordomo@vger.kernel.org) by vger.kernel.org via listexpand
        id S229936AbjGJQS6 (ORCPT <rfc822;lists+linux-nfs@lfdr.de>);
        Mon, 10 Jul 2023 12:18:58 -0400
Received: from lindbergh.monkeyblade.net ([23.128.96.19]:36274 "EHLO
        lindbergh.monkeyblade.net" rhost-flags-OK-OK-OK-OK) by vger.kernel.org
        with ESMTP id S229659AbjGJQS4 (ORCPT
        <rfc822;linux-nfs@vger.kernel.org>); Mon, 10 Jul 2023 12:18:56 -0400
Received: from dfw.source.kernel.org (dfw.source.kernel.org [IPv6:2604:1380:4641:c500::1])
        by lindbergh.monkeyblade.net (Postfix) with ESMTPS id C2F5F106
        for <linux-nfs@vger.kernel.org>; Mon, 10 Jul 2023 09:18:54 -0700 (PDT)
Received: from smtp.kernel.org (relay.kernel.org [52.25.139.140])
        (using TLSv1.3 with cipher TLS_AES_256_GCM_SHA384 (256/256 bits)
         key-exchange X25519 server-signature RSA-PSS (2048 bits))
        (No client certificate requested)
        by dfw.source.kernel.org (Postfix) with ESMTPS id 58681610A2
        for <linux-nfs@vger.kernel.org>; Mon, 10 Jul 2023 16:18:54 +0000 (UTC)
Received: by smtp.kernel.org (Postfix) with ESMTPSA id 6AB56C433C7;
        Mon, 10 Jul 2023 16:18:53 +0000 (UTC)
DKIM-Signature: v=1; a=rsa-sha256; c=relaxed/simple; d=kernel.org;
        s=k20201202; t=1689005933;
        bh=wdkhIuvDoeHkiF2MfN1z3aKdqv4w4UDhVdvYaPtpGMY=;
        h=Date:From:To:Cc:Subject:References:In-Reply-To:From;
        b=
Date:   Mon, 10 Jul 2023 18:18:50 +0200
From:   Lorenzo Bianconi <lorenzo@kernel.org>
To:     Chuck Lever <cel@kernel.org>
Cc:     linux-nfs@vger.kernel.org, lorenzo.bianconi@redhat.com,
        chuck.lever@oracle.com, jlayton@kernel.org
Subject: Re: [RFC] NFSD: add rpc_status entry in nfsd debug filesystem
Message-ID: <ZKwvagKhuHlzA4+x@lore-desk>
References: <bac972c22c5acfa43969bb1bf164341b16ea045c.1688032742.git.lorenzo@kernel.org>
 ...
 <ZJ39OqQJZQw8vsXh@manet.1015granger.net>
MIME-Version: 1.0
Content-Type: multipart/signed; micalg=pgp-sha512;
        protocol="application/pgp-signature"; boundary="A42N2qrieyu7CcMy"
Content-Disposition: inline
In-Reply-To: <ZJ39OqQJZQw8vsXh@manet.1015granger.net>
...
> > >  };
> > > =20
> > > @@ -195,6 +197,21 @@ static inline struct net *netns(struct file *fil=
e)
...
> > > +{
> > > +	struct nfsd_net *nn =3D net_generic(file_inode(m->file)->i_sb->s_fs=
_info,
> > > +					  nfsd_net_id);
> > > +
...
> > >   *
> > > @@ -1400,6 +1417,7 @@ static int nfsd_fill_super(struct super_block *=
sb, struct fs_context *fc)
> > >  		[NFSD_RecoveryDir] =3D {"nfsv4recoverydir", &transaction_ops, S_IW=
USR|S_IRUSR},
> > >  		[NFSD_V4EndGrace] =3D {"v4_end_grace", &transaction_ops, S_IWUSR|S=
_IRUGO},
> > >  #endif
> > > +		[NFSD_Rpc_Status] =3D {"rpc_status", &nfsd_rpc_status_fops, S_IRUG=
O},
> > >  		/* last one */ {""}
> > >  	};
> > > =20
...
> > > --- a/fs/nfsd/nfssvc.c
> > > +++ b/fs/nfsd/nfssvc.c
> > > @@ -1142,3 +1142,64 @@ int nfsd_pool_stats_release(struct inode *inod=
e, struct file *file)
> > >  	mutex_unlock(&nfsd_mutex);
> > >  	return ret;
> > >  }
...
> > > =20
> > >  /**
> > >   * svc_encode_result_payload - mark a range of bytes as a result pay=
load
> > > --=20
> > > 2.41.0
> > >=20
...
iHUEABYKAB0WIQTquNwa3Txd3rGGn7Y6cBh0uS2trAUCZKwvaQAKCRA6cBh0uS2t
rOWaAQDADhOicJpCcQQR3u0zT+kvNPY3Rx5IAD6oWYHnqbbPewEAzgEfywExVBY3
TjRuEqOG2tYF6SC0qDN2F22YT/mQ1wM=
=RVVK
-----END PGP SIGNATURE-----
