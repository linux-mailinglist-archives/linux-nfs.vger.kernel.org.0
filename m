Return-Path: <linux-nfs+bounces-128-lists+linux-nfs=lfdr.de@vger.kernel.org>
X-Original-To: lists+linux-nfs@lfdr.de
Delivered-To: lists+linux-nfs@lfdr.de
Received: from sv.mirrors.kernel.org (sv.mirrors.kernel.org [139.178.88.99])
	by mail.lfdr.de (Postfix) with ESMTPS id EDCBF7FBC7F
	for <lists+linux-nfs@lfdr.de>; Tue, 28 Nov 2023 15:15:46 +0100 (CET)
Received: from smtp.subspace.kernel.org (wormhole.subspace.kernel.org [52.25.139.140])
	(using TLSv1.2 with cipher ECDHE-RSA-AES256-GCM-SHA384 (256/256 bits))
	(No client certificate requested)
	by sv.mirrors.kernel.org (Postfix) with ESMTPS id A410E28292A
	for <lists+linux-nfs@lfdr.de>; Tue, 28 Nov 2023 14:15:45 +0000 (UTC)
Received: from localhost.localdomain (localhost.localdomain [127.0.0.1])
	by smtp.subspace.kernel.org (Postfix) with ESMTP id 232445ABAC;
	Tue, 28 Nov 2023 14:15:42 +0000 (UTC)
Authentication-Results: smtp.subspace.kernel.org;
	dkim=pass (2048-bit key) header.d=kernel.org header.i=@kernel.org header.b="VavKX3Sq"
X-Original-To: linux-nfs@vger.kernel.org
Received: from smtp.kernel.org (aws-us-west-2-korg-mail-1.web.codeaurora.org [10.30.226.201])
	(using TLSv1.2 with cipher ECDHE-RSA-AES256-GCM-SHA384 (256/256 bits))
	(No client certificate requested)
	by smtp.subspace.kernel.org (Postfix) with ESMTPS id F25035AB98;
	Tue, 28 Nov 2023 14:15:41 +0000 (UTC)
Received: by smtp.kernel.org (Postfix) with ESMTPSA id 330A1C433C7;
	Tue, 28 Nov 2023 14:15:41 +0000 (UTC)
DKIM-Signature: v=1; a=rsa-sha256; c=relaxed/simple; d=kernel.org;
	s=k20201202; t=1701180941;
	bh=gFc993CzQqumBq2cClxHXIUNWb+1m9JTJwRV/t0uK30=;
	h=Subject:From:To:Cc:Date:In-Reply-To:References:From;
	b=
Message-ID: <518775f9f9bd3ad1afec0bde4d0a6bee3370bdd4.camel@kernel.org>
Subject: Re: [PATCH/RFC] core/nfsd: allow kernel threads to use task_work.
From: Jeff Layton <jlayton@kernel.org>
To: Christian Brauner <brauner@kernel.org>, NeilBrown <neilb@suse.de>
Cc: Al Viro <viro@zeniv.linux.org.uk>, linux-fsdevel@vger.kernel.org, 
	linux-kernel@vger.kernel.org, linux-nfs@vger.kernel.org
Date: Tue, 28 Nov 2023 09:15:39 -0500
...
> > > >=20
> > > > I have evidence from a customer site of 256 nfsd threads adding fil=
es to
> > > > delayed_fput_lists nearly twice as fast they are retired by a singl=
e
> > > > work-queue thread running delayed_fput().  As you might imagine thi=
s
> > > > does not end well (20 million files in the queue at the time a snap=
shot
> > > > was taken for analysis).
> > > >=20
> > > > While this might point to a problem with the filesystem not handlin=
g the
> > > > final close efficiently, such problems should only hurt throughput,=
 not
...
?h=3Dnfsd-next&id=3Dc42661ffa58acfeaf73b932dec1e6f04ce8a98c0
> > >=20
> >=20
> > Thanks....
...
> > my patch isn't so important now.  Particularly as nfsd now isn't closin=
g
...
> > together into a list and closing them sequentially is not going to be a=
s
> > efficient as closing them in parallel.
> >=20
> > >=20
> > > > For normal threads, the thread that closes the file also calls the
> > > > final fput so there is natural rate limiting preventing excessive g=
rowth
> > > > in the list of delayed fputs.  For kernel threads, and particularly=
 for
> > > > nfsd, delayed in the final fput do not impose any throttling to pre=
vent
...
I struggle to think about how we would document a new option like this.=A0
...
> > have each "nfsd_open" check if there are pending closes, and to wait fo=
r
