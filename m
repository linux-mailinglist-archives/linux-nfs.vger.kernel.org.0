...
Received: from out1.vger.email (out1.vger.email [IPv6:2620:137:e000::1:20])
	by mail.lfdr.de (Postfix) with ESMTP id 4671074E2ED
	for <lists+linux-nfs@lfdr.de>; Tue, 11 Jul 2023 03:06:11 +0200 (CEST)
Received: (majordomo@vger.kernel.org) by vger.kernel.org via listexpand
        id S229990AbjGKBGJ (ORCPT <rfc822;lists+linux-nfs@lfdr.de>);
        Mon, 10 Jul 2023 21:06:09 -0400
Received: from lindbergh.monkeyblade.net ([23.128.96.19]:49328 "EHLO
        lindbergh.monkeyblade.net" rhost-flags-OK-OK-OK-OK) by vger.kernel.org
        with ESMTP id S229655AbjGKBGI (ORCPT
        <rfc822;linux-nfs@vger.kernel.org>); Mon, 10 Jul 2023 21:06:08 -0400
Received: from dfw.source.kernel.org (dfw.source.kernel.org [139.178.84.217])
        by lindbergh.monkeyblade.net (Postfix) with ESMTPS id 8397F12A
        for <linux-nfs@vger.kernel.org>; Mon, 10 Jul 2023 18:06:05 -0700 (PDT)
Received: from smtp.kernel.org (relay.kernel.org [52.25.139.140])
        (using TLSv1.3 with cipher TLS_AES_256_GCM_SHA384 (256/256 bits)
         key-exchange X25519 server-signature RSA-PSS (2048 bits))
        (No client certificate requested)
        by dfw.source.kernel.org (Postfix) with ESMTPS id 12E37612B1
        for <linux-nfs@vger.kernel.org>; Tue, 11 Jul 2023 01:06:05 +0000 (UTC)
Received: by smtp.kernel.org (Postfix) with ESMTPSA id D3817C433C7;
        Tue, 11 Jul 2023 01:06:03 +0000 (UTC)
DKIM-Signature: v=1; a=rsa-sha256; c=relaxed/simple; d=kernel.org;
        s=k20201202; t=1689037564;
        bh=NFFJ75C1Fh23fa+dJqqJh7T68VPvw4GbuuOcBPjXjQQ=;
        h=Subject:From:To:Cc:Date:In-Reply-To:References:From;
        b=
Message-ID: <2909e8cfc2cbd218372e78f5e215759722faba51.camel@kernel.org>
...
Date:   Mon, 10 Jul 2023 21:06:02 -0400
In-Reply-To: <0D6735B0-77A8-4710-8EE7-1F8E382A139B@oracle.com>
...
> > > - * exist and each must be listed in ->sp_all_threads in some entry o=
f
> > > - * ->sv_pools[].
...
> > > + ),
> > > +
> > > + TP_printk("service=3D%s pool=3D%d pool_flags=3D%s nrthreads=3D%u th=
read_id=3D%u",
...
> > > --- a/net/sunrpc/svc.c
> > > +++ b/net/sunrpc/svc.c
> > > @@ -507,8 +507,8 @@ __svc_create(struct svc_program *prog, unsigned i=
...
> > > if (!rqstp)
> > > @@ -687,11 +693,21 @@ svc_prepare_thread(struct svc_serv *serv, struc=
t svc_pool *pool, int node)
...
> > >=20
> > > /**
> > > @@ -708,19 +724,17 @@ struct svc_rqst *svc_pool_wake_idle_thread(stru=
ct svc_serv *serv,
...
> > returns needs some protection too. Between xa_for_each returning a rqst=
p
...
> > waking a thread that's already on its way out of the pool? Maybe this i=
s
> > addressed in your next patch though...
> >=20
> > > trace_svc_pool_starved(serv, pool);
> > > percpu_counter_inc(&pool->sp_threads_starved);
> > > @@ -736,32 +750,33 @@ svc_pool_next(struct svc_serv *serv, struct svc=
_pool *pool, unsigned int *state)
> > > static struct task_struct *
> > > svc_pool_victim(struct svc_serv *serv, struct svc_pool *pool, unsigne=
d int *state)
> > > {
> > > - unsigned int i;
...
> > > - struct svc_rqst *rqstp;
> > > -
> > > - rqstp =3D list_entry(pool->sp_all_threads.next, struct svc_rqst, rq=
_all);
...
> > > }
> > >=20
> > > @@ -843,9 +858,9 @@ svc_set_num_threads(struct svc_serv *serv, struct=
