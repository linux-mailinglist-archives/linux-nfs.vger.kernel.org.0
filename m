Return-Path: <linux-nfs+bounces-227-lists+linux-nfs=lfdr.de@vger.kernel.org>
X-Original-To: lists+linux-nfs@lfdr.de
Delivered-To: lists+linux-nfs@lfdr.de
Received: from sv.mirrors.kernel.org (sv.mirrors.kernel.org [139.178.88.99])
	by mail.lfdr.de (Postfix) with ESMTPS id C86987FF8F8
	for <lists+linux-nfs@lfdr.de>; Thu, 30 Nov 2023 19:01:20 +0100 (CET)
Received: from smtp.subspace.kernel.org (wormhole.subspace.kernel.org [52.25.139.140])
	(using TLSv1.2 with cipher ECDHE-RSA-AES256-GCM-SHA384 (256/256 bits))
	(No client certificate requested)
	by sv.mirrors.kernel.org (Postfix) with ESMTPS id 5224D28174F
	for <lists+linux-nfs@lfdr.de>; Thu, 30 Nov 2023 18:01:19 +0000 (UTC)
Received: from localhost.localdomain (localhost.localdomain [127.0.0.1])
	by smtp.subspace.kernel.org (Postfix) with ESMTP id 599E6584DF;
	Thu, 30 Nov 2023 18:01:17 +0000 (UTC)
Authentication-Results: smtp.subspace.kernel.org;
	dkim=pass (2048-bit key) header.d=kernel.org header.i=@kernel.org header.b="gHNJCkZF"
X-Original-To: linux-nfs@vger.kernel.org
Received: from smtp.kernel.org (aws-us-west-2-korg-mail-1.web.codeaurora.org [10.30.226.201])
	(using TLSv1.2 with cipher ECDHE-RSA-AES256-GCM-SHA384 (256/256 bits))
	(No client certificate requested)
	by smtp.subspace.kernel.org (Postfix) with ESMTPS id 356A058103;
	Thu, 30 Nov 2023 18:01:16 +0000 (UTC)
Received: by smtp.kernel.org (Postfix) with ESMTPSA id F10C1C433C7;
	Thu, 30 Nov 2023 18:01:15 +0000 (UTC)
DKIM-Signature: v=1; a=rsa-sha256; c=relaxed/simple; d=kernel.org;
	s=k20201202; t=1701367276;
	bh=SnqyGMXUYlVkn27q/xHttRGCXJ4TpthT1u5RBW9Kt7I=;
	h=Subject:From:To:Cc:Date:In-Reply-To:References:From;
	b=
Message-ID: <0eaec872db97e1b7366fc2b673484358312404b0.camel@kernel.org>
...
In-Reply-To: <ZWjI6ppENVu2FPIo@tissot.1015granger.net>
...
> > > > > >=20
> > > > > > +/* =3D=3D=3D=3D=3D=3D=3D=3D=3D=3D=3D=3D=3D=3D NFSD_CMD_LISTENE=
R_START =3D=3D=3D=3D=3D=3D=3D=3D=3D=3D=3D=3D=3D=3D */
...
> > > > > > +};
> > > > >=20
> > > > > How do you deconfigure a listener with this interface? i.e. suppo=
se I
> > > > > want to stop nfsd from listening on a particular port? I think th=
is too
> > > > > is a place where a declarative interface would be better:
> > > >=20
> > > > Is it possible with current APIs? as for 2/3 so far I have just add=
ed netlink
> > > > counter for current implementation but I am fine to change the logi=
c here to
> > > > better APIs.
> > > >=20
> > > > >=20
...
> > >=20
> > > > > Have userland send down a list of the ports that we should curren=
tly be
...
> > the address to listen on.=A0Later we added a way to just hand off a soc=
ket
> > to the kernel to pass that info.
> >=20
> > I think it's possible today to send down a socket that only listens on =
a
